...
//     出すのは、エラーを出すより悪い。
//   ・**結果の遅延再送（Background Sync）は入れない**。この判断は
//     サーバーに冪等キー(runId)が入ったいまも変わらない ── ただし理由は
//     変わった。以前は「二重加算になるから」だったが、いまは
//     POST /api/game/result が同じ runId を2回受けても1回ぶんしか
//     数えないので、二重加算そのものは起きない。
//     それでも入れないのは、**控えを1箇所に集めておきたい**から。
//     圏外で遊んだぶんは public/js/net.js が持ち、つながったら送り直す。
//     Service Worker は画面もセッションも無いところから動くので、
//     そこから報酬の付く POST を投げる仕組みを増やすと、控えの持ち主が
//     2つになり「どちらが送ったか分からない」状態を作ることになる。
//     （test/offline.test.mjs が、sw.js が /api/game/result を
//       知らないままであることを見張っている。）
const VERSION = 'v2';
