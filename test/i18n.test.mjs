...
const MUST_HAVE = [
  ...
  '所持していないアイテムです',
  'まだ獲得していない称号です',
  // v2.34: 登録／改名で名前が弾かれる理由（使用中／住人と同名／予約名）を
  // 出し分けるのをやめた ── 出し分けは「どの名前が住人か」を総当たりできる
  // 列挙オラクルだった。3分岐とも同じ 409 ＋ この1文になったので、見張る
  // 対象もこちらへ移す（'その名前は使えません。別の名前でどうぞ' は
  // サーバーから消えたので、ここに残すと「死んだ対訳」検査が落ちる）。
  'そのユーザー名は既に使われています',
