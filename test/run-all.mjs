...
const TESTS = [
  ...
  'audit.test.mjs',
  // 🔄 交換所。コインとジェムの出口。
  //   ★ B節が本命 ── 交換所限定が**他の3経路（棚・ガチャ・図鑑）に漏れない**こと。
  //   入手経路を足すと必ずどこかが漏れる（このリポジトリで最も多い事故の形）。
  'exchange.test.mjs',
  // 🚪 入手経路の関門。**server/catalog.js が名指しで「これが見張っている」と
  //   書いているのに、このファイルは長らく存在していなかった。**
  //   ★ B-1 が本題 ── 経路の組み合わせ（!adminOnly && !throneOnly && …）を
  //   catalog.js の外で手書きしていないこと。以前18か所に写しがあって、
  //   1つ忘れると「ガチャで交換所限定が出る」形で静かに壊れていた。
  //   実際にこのテストは、実績の母数が図鑑と21種ずれているのを見つけている。
  'catalog-gates.test.mjs',
  // 🎨 相手の盤面を相手のスキンで描く（v2.72）。
  //   ★ A-2 と B-1 が本題。A-2 は「住人が既定ばかりでも全員着替えでもない」
  //   ── どちらかに寄ると、それだけで住人を選り分けられる。
  //   B-1 は名簿の指紋。スキンを名前から決め打ちしているのは、makeResident の
  //   乱数列に1回でも足すと以降の住人が全部ずれるから（実際に1回足して確認済み）。
  'oppskin.test.mjs',
  // 🪟 相手カードの並びと、⏳「時間が解決する」結果の控え（v2.74）。
  //   ★ A-3/A-4 と B-1/B-2 が本題。
  //   A: #oppCards が折り返さない1行だったので、16人ルーム（相手15人）で
  //      実測27組が重なっていた。8人までは1行のまま・15人でも2行に収める。
  //   B: 結果の控えは status 0 だけ作られるのに、送り直しは 0/429/503 を
  //      再試行していた（＝1回目だけ捨てる）。再デプロイ中の 503 で
  //      「この回の報酬は付いていません」が出ていた原因。表を1つにした。
  'opplayout.test.mjs',
  // 🧾 端末に残る記録／📜 実績の受け取りの絞り／🎰 コンプ後のガチャ（v2.79）。
  //   ★ A-3 と B-2 が本題。
  //   A-3: 取り消しは仕舞い（bba_arch:<持ち主>）からも消す ── 残すと
  //        ログインし直した瞬間に自己ベストが戻ってくる。
  //   B-2: 1日の枠が尽きていても大物の実績は1件通す ── 通さないと、
  //        上限より高い実績が永久に受け取れなくなる。
  'records-device.test.mjs',
  // 🎪 にぎわい倍率（v2.80）。上げても増えなかった天井と、混んだ世界でだけ
  //   開く秘匿の穴。★ A-2（待ち時間が定数にならない）と C-1（ギルド加入率が
  //   住人と使い捨てを見分ける統計にならない）が本題。
  //   ⚠ 名簿の上限を動かすときは、追随する側（chatgen の記憶・戦績台帳・
  //     ギルドの席数）も一緒に見ること。B節と C節がそれを押さえている。
  'nigiwai.test.mjs',
  // 🗣 住人の口と、板の上の事実が食い違っていないか（v2.82）。
  //   ★ A-1/A-2 が本題 ── 住人は深淵に構造的に入れないのに、話題9本のうち
  //   6本が一人称の体験談で、1本は「塔100階クリアで解放された」と
  //   **絶対に持てない記録**を名乗っていた。速報・実績・称号・順位表では
  //   同じ不変条件を止めてあるのに、チャットの層だけ関門が無かった。
  'crowdtruth.test.mjs',
  // 🌳 盤面の動く風景と、🏮 運営専用の新しい装備（v2.69）。
  //   ★ A-4 が本命 ── 風景は**別の紙に描いてから1回だけ薄くして重ねる**。
  //   「globalAlpha は 0.30 まで」という作者の約束では守れなかった（図形が重なると
  //   α が二重に乗り、実測で 街0.48 / 鳥居0.41 / 波0.32 と上限を超えていた）。
  //   盤面に重ねる合図（消える線の白帯・置けないマスの赤）が沈むと遊べなくなる。
  //   canvas が要らない検査だけなので安い組。実際の濃さは画面で測ること。
  'scenes.test.mjs',
