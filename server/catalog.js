...
// ⚠ 有料枠のジェムは**払った額より少なく**すること。
//
//   500💎 払って 620💎 返っていた（＝ +120💎 の純増）。しかも2周目以降は
//   すでに持っている skin_gold(250💎) が「同じ値段ぶんの通貨」に振り替わり、
//   その振替先がジェムだったので **870💎 返って +370💎**。
//   ジェムの出口は棚と交換所の一度きり（計5,110💎）しか無く、
//   1日120💎 入るので、唯一の周回する出口だったはずのバトルパスが
//   実際には**いちばん大きなジェムの蛇口**になっていた。
//
//   減らしたぶんは**コインに振り替えて**ある（22.8🪙/💎＝ガチャの実効レート）。
//   もらえる総額は変えず、「ジェムで買ってジェムが増える」形だけをやめる。
//   ⚠ 受け取りは tier:track ごとに記録されるので、表を変えても
//     **受け取り済みのぶんには影響しない**（未受取の段から新しい値になる）。
