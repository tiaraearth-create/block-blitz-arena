...
export const EVENT_TYPES = [
  // 🌪️ 「カオスタイム」は **引退させた**（2026-09-06）。
  //
  //   もともとの中身は2つ ──「カオスモードを遊べるようにする」と「コイン1.5倍」。
  //   開放は v2.48 で常時になり、倍率は「1倍にしてほしい」という指示で v2.54 に
  //   やめたので、残っていたのは `bonus: {}` ＝ **何も起きないイベント**だった。
  //   他の7種はどれも実入りか手触りを変えるので、これだけが「開催中！」と
  //   告知されて何も起きない枠になり、運営が選ぶ一覧でも罠になっていた。
  //
  //   足せなかった理由も残しておく（次に考える人のために）:
  //     ・コイン/XP/ジェム/ガチャ … 既に別のイベントが持っている枠で、
  //       しかも通貨を増やす方向はユーザーの指示（1倍）と逆を向く。
  //     ・カオスだけを強くする（リロール増など）… カオスは scoreboardEligible なので
  //       ハイスコアの板と王座に反映される。イベント日だけ有利にすると、
  //       **その日の記録だけ他の日と比べられなくなる**。
  //   カオスモード自体は常時遊べるままで、何も失われていない。
  //   復活させるなら、上の2点を解いてから EVENT_TYPES に書き戻すこと。
  {
